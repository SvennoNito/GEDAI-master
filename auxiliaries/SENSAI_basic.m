...
function [SENSAI_score, SIGNAL_subspace_similarity, NOISE_subspace_similarity, mean_ENOVA, ENOVA_per_epoch, signal_silhouette, SENSAI_normalized, RSA_score] = SENSAI_basic(signal_data, noise_data, srate, epoch_size, refCOV, NOISE_multiplier, signal_type)
...
RSA_Distribution = zeros(1, num_epochs);
...
for epoch = 1:num_epochs
    ...
    % Relative Subspace Alignment (RSA) using Standard SSI
    [Vraw, Draw] = eig(cov(original_epoch'));
    [~, idxRaw] = sort(diag(Draw), 'descend');
    basis_raw = Vraw(:, idxRaw(1:SSI_top_PCs));
    ssi_raw_std = prod(subspace_angles(basis_raw, basis_ref));
    
    ssi_clean_std = SIGNAL_subspace_similarity_distribution(epoch);
    ssi_noise_std = NOISE_subspace_similarity_distribution(epoch);
    
    if (ssi_clean_std - ssi_noise_std) < 1e-6
        RSA_Distribution(epoch) = 0;
    else
        RSA_Distribution(epoch) = (ssi_raw_std - ssi_noise_std) / (ssi_clean_std - ssi_noise_std);
    end
end

mean_ENOVA = mean(ENOVA_per_epoch);
RSA_score = mean(RSA_Distribution);
